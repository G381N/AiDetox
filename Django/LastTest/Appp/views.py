...
class MainHandler(APIView):
    def get(self,request):
        data = request.GET
        if "id" in data or "fname" in data:
            menu = Menu.objects.filter(_id = data['id']).first() or Menu.objects.filter(fname = data['fname']).first() 
    ...
    def patch(self,request):
        ...
        if S.is_valid():
            S.save() #Updating
            return Response(S.data,status=status.HTTP_201_CREATED) #Response with Success
        return Response(S.errors,status=status.HTTP_400_BAD_REQUEST) #Response with Error



#Changes Made : Changed the status to use the rest_framework status.
